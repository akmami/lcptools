 ...
 * I/O operations on bit sequences.
 *
 ...
 * @struct core
 *
 * @note Define `STATS` before including this file to enable the tracking of
 * start and end indices for sequences.
...
namespace lcp {
	...
	struct core {
	  public:
		...
		template <typename Iterator, typename Size, typename Representation, typename Data>
		core(Iterator begin, Iterator end, std::pair<size_t, size_t> indeces, Size size, Representation rep, Data data, bool use_map) {
			...
			memset(this->bit_rep, 0, block_number * sizeof(ublock));
			...
			if (!use_map) {
				this->label = hash::simple(data(begin, end));
			} else {
				this->label = hash::emplace(data(begin, end));
			}
		};
		 ...
		 *
		 * This constructor initializes a `core` object with a given pointer to
		 * a bit sequence, and sets its size, start, and end indices.
		 *
		 ...
		 */
		core(size_t bit_size, ublock *bit_rep, ulabel label, size_t start, size_t end);

		/**
		 * @brief Constructs a `core` object by reading from an input file
		 * stream.
		 *
		 * This constructor reads a `core` object's metadata and bit sequence
		 * from a file. It allocates memory and reads the bit sequence into the
		 * object.
		 *
		...
		/**
		 * @brief Compresses the current `core` object by comparing it with
		 * another `core` object.
		 *
		 * This function compresses the current bit sequence by identifying
		 * common patterns between the current object and another `core` object,
		 ...
		 *
		 * This function writes the metadata and bit sequence of the `core`
		 * object to a file.
		 *
		 ...
		 *
		 * This operator performs a deep copy of the `other` core's data into
		 * the current instance. It first checks for self-assignment, then
		 * deallocates any existing memory pointed to by `bit_rep`. If
		 * `other.bit_rep` is not null, it allocates new memory and copies the
		 * data from `other`'s `bit_rep` array. If `other.bit_rep` is null, it
		 * sets `bit_rep` to nullptr. Additionally, it copies the `size` and
		 * `label` values from `other`. If `STATS` is defined, it also copies
	 ...
	 * @param lhs The left-hand side `core` object.
	 * @param rhs The right-hand side `core` object.
	 * @return True if the left-hand object is greater than or equal, false
	 * otherwise.
	 */
	bool operator>=(const struct core &lhs, const struct core &rhs);
	 ...
	 * @param lhs The left-hand side `core` object.
	 * @param rhs The right-hand side `core` object.
	 * @return True if the left-hand object is smaller than or equal, false
	 * otherwise.
	 */
	bool operator<=(const struct core &lhs, const struct core &rhs);

	/**
	 * @brief Overloads the stream insertion operator (<<) to output the bit
	 * representation of a `core` object.
	 *
	 ...
	 *
	 * @param os The output stream to which the bits of the `core` object will
	 * be written.
	 * @param element The `core` object to be output.
	 * @return A reference to the output stream `os` after the `core` object has
	 * been written.
	 */
	...
	/**
	 * @brief Overloads the stream insertion operator (<<) to output the bit
	 * representation of a `core` pointer.
	 *
	 * This function iterates over the bits in the `core` object pointed to by
	 * the given pointer and outputs them to the given output stream. It prints
	 * each bit as either 0 or 1, starting from the most significant bit to the
