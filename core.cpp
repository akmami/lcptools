...
namespace lcp {
	...
	core::core( size_t start, size_t end, size_t bit_size, ublock* bit_rep, uint32_t label ) {
		#ifdef STATS
		this->start = start;
		this->end = end;
		#else
	...
	void core::compress( const struct core& other ) {
		...
		uint32_t index = std::min(this->bit_size, other.bit_size);
		uint32_t t_block = ( this->bit_size - 1 ) / UBLOCK_BIT_SIZE, o_block = ( other.bit_size - 1 ) / UBLOCK_BIT_SIZE;
	...
	bool operator > ( const struct core& lhs, const struct core& rhs ) {
		...
		while ( index < lhs.bit_size ) {
			if ( lhs.bit_rep[index / UBLOCK_BIT_SIZE ] > rhs.bit_rep[index / UBLOCK_BIT_SIZE ] ) {
				return true;
			} else if ( lhs.bit_rep[index / UBLOCK_BIT_SIZE ] < rhs.bit_rep[index / UBLOCK_BIT_SIZE ] ) {
				return false;
			}
